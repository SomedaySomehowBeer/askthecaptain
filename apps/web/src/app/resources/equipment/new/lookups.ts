...
export async function reservationLookups(token: string, org: string) {
	...
	if (commitments.ok) {
		for (const p of commitments.value.projects) labels[p.id] = p.systemKind === 'obligations' ? 'Obligations' : p.name;
	...
	const options: ReservationOptions = {
		...
		// Only what a booking may link: top-level, not cancelled, in an active project.
		tasks: commitments.ok ? commitments.value.tasks.filter((t) => !t.parentId && t.status !== 'cancelled' && active.some((p) => p.id === t.projectId))
