...
export async function ShopifyStock({ me, offset = 0 }: { me: Awaited<ReturnType<typeof requireCurrent>>; offset?: number }) {
 ...
 return <div className="stack" id="shopify-stock"><h3>Shop stock</h3>
  {!result.ok ? <Notice tone="failed">{result.error.message} Shopify stock could not be read. <a href="/commitments">Try again</a>.</Notice> : <>
   {!result.value.connected ? <Notice>Connect Shopify in <a href="/settings/connections">Settings → Connections</a> to read shop quantities here.</Notice> : <>
    ...
    <div className="row">{offset > 0 ? <a href={`/commitments?shopifyOffset=${Math.max(0, offset - 200)}#shopify-stock`}>Previous shop stock</a> : null}{result.value.nextOffset !== null ? <a href={`/commitments?shopifyOffset=${result.value.nextOffset}#shopify-stock`}>More shop stock</a> : null}</div>
   </>}
  </>}
 </div>;
}
