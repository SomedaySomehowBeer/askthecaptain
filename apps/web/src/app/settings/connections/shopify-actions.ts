...
export async function shopifyAction(form: FormData): Promise<{ error?: string; authorizationUrl?: string }> {
 ...
 try {
  ...
  if (operation === 'sync' || operation === 'disconnect') { await api(`${root}/${operation === 'sync' ? 'sync' : 'connection'}`, { token: me.token, method: operation === 'sync' ? 'POST' : 'DELETE' }); revalidatePath('/settings/connections'); revalidatePath('/commitments'); revalidatePath('/resources/inventory'); return {}; }
  if (operation === 'reorder') { await api(`${root}/stock/${encodeURIComponent(String(form.get('itemId')))}`, { token: me.token, method: 'PATCH', body: { reorderPoint: String(form.get('reorderPoint') ?? '').trim() || null } }); revalidatePath('/settings/connections'); revalidatePath('/commitments'); revalidatePath('/resources/inventory'); return {}; }
