...
export function NewTaskForm({ owners, ownerId, projects, projectId }: { owners: OwnerOption[]; ownerId: string; projects: ProjectOption[] | null; projectId: string }) {
	...
	function submit(event: FormEvent<HTMLFormElement>) {
		...
		startTransition(async () => {
			...
			} catch { setError('The save could not be confirmed. Check Work before trying again.'); }
		});
	}
	return (
		<form className="form" onSubmit={submit} method="post" aria-busy={pending || undefined}>
			<fieldset disabled={pending} className="work-new__fields">
			...
			<div className="row">
				<button className="button button--primary" type="submit" disabled={pending}>{pending ? 'Adding…' : 'Add task'}</button>
