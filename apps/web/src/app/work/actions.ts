...
export async function createWorkTask(form: FormData): Promise<CreateResult> {
	...
	if (!me?.organisation) return { error: 'Your session has ended. Sign in again, then add the task.' };
	...
	try {
		const task = await api<Task>(`/v1/organisations/${me.organisation.organisationId}/tasks`, { method: 'POST', token: me.token,
