...
import { attachmentsInWords, mailDay, mailTime, type MailList, type ThreadSummary } from './mail.ts';
...
async function Inbox({ me, before }: { me: Awaited<ReturnType<typeof requireCurrent>>; before?: string }) {
	...
	const { connection, lastSync, threads, timezone, nextBefore, automaticSyncEnabled } = result.value;
	...
	for (const thread of threads) { const day = mailDay(thread.sentAt, timezone); groups.set(day, [...(groups.get(day) ?? []), thread]); }
	return <>
		...
		{[...groups].map(([day, items]) => <section className="card" key={day}><h2>{day}</h2><ul className="bare">
			{items.map((thread) => <li className="mail-row" key={thread.id}><Link className="mail-link" href={`/inbox/${thread.id}`}>
				...
				<h3>{thread.subject || '(No subject)'}</h3><p className="secondary">{thread.snippet || 'No preview available.'}</p>
