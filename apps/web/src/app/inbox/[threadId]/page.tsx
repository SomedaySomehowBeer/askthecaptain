...
async function Thread({ me, id }: { me: Awaited<ReturnType<typeof requireCurrent>>; id: string }) {
	...
	return <>
		...
		{thread.messages.map((m) => <article className="card stack mail-message" key={m.id}>
			<h2>{m.subject || '(No subject)'}</h2><div className="stack secondary"><p><strong>From:</strong> {m.fromHeader || 'Sender unavailable'}</p><p><strong>To:</strong> {m.toHeader || 'Recipients unavailable'}</p>
