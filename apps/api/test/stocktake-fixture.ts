...
export async function stocktakeFixture(db: Harness) {
 const f = await triageFixture(db); const pushes: Payload[] = [];
 await f.workflows.enable(f.actor, f.org, 'inbox-triage', { enabled: false });
 ...
 const stocktake = new StocktakeService(db.app, f.inference, push); stocktake.register(f.registry);
