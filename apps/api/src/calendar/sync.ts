...
export class CalendarSync {
 ...
 async run(organisationId: string): Promise<CalendarSyncResult> {
  ...
  const counts: CalendarSyncResult = { calendars: 0, events: 0, cancelled: 0, full: 0 }; let stage = 'start';
  const batch = <T>(work: (tx: TransactionSql) => Promise<T>) => withTenant(this.db, { organisationId }, async (tx) => {
   ...
   if (current?.status !== 'connected' || current.accountEmail !== conn!.accountEmail) throw new CalendarError(502, 'account_changed');
   return work(tx);
  });
  try {
   conn = await withTenant(this.db, { organisationId }, connection);
   if (!conn || conn.status !== 'connected') throw new CalendarError(502, 'not_connected');
   ...
   stage = 'calendars'; const listed: CalendarInfo[] = []; let pageToken: string | undefined; const pages = new Set<string>();
   do {
    const page = await this.client.calendars(token, pageToken); listed.push(...page.items); pageToken = page.nextPageToken;
    if (listed.length > 1000 || (pageToken && pages.has(pageToken))) throw new CalendarError(502, 'too_many_pages');
    if (pageToken) pages.add(pageToken); if (pages.size > 100) throw new CalendarError(502, 'too_many_pages');
    await batch(async () => {});
   } while (pageToken);
   stage = 'save'; const selected = await batch(async (tx) => {
   ...
   for (const c of selected) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
     ...
     try {
      do {
       stage = 'events'; const page = await this.client.events(token, c.providerId, { ...(full ? { timeMin: window.from, timeMax: window.to } : { syncToken: previous!.syncToken }), ...(pageToken ? { pageToken } : {}) });
       if (page.nextPageToken && (seenPages.has(page.nextPageToken) || seenPages.size >= 100)) throw new CalendarError(502, 'too_many_pages');
       stage = 'save'; await batch(async (tx) => {
        if (full && first) {
   ...
   stage = 'finish'; await batch((tx) => audit(tx, { organisationId, actor: { kind: 'system' }, action: 'calendar.synced', subjectType: 'connection', subjectId: conn!.id, detail: { ...counts, success: true } }));
   return counts;
  } catch (error) {
   if (error instanceof HttpError && error.code === 'calendar_sync_running') throw error;
   const failure = describeFailure(error, stage);
   const message = error instanceof HttpError ? error.message : `${failed} ${explainFailure('Google Calendar', failure)}`;
   await withTenant(this.db, { organisationId }, (tx) => audit(tx, { organisationId, actor: { kind: 'system' }, action: 'calendar.sync_failed', subjectType: 'organisation', subjectId: organisationId,
    detail: { ...counts, success: false, error: message, failure } }));
   throw new HttpError(503, 'calendar_sync_failed', message);
  } finally {
