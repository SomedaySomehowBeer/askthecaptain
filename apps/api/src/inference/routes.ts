...
export function inferenceRoutes(service?: InferenceService) {
 ...
 routes.post('/v1/organisations/:id/inference/runtime/login/code', async c => {
  ...
  const input = z.object({ code: z.string().max(1024).transform(v => v.replace(/https?:\/\/[\s\S]*$/, '').replace(/[^A-Za-z0-9_#.:-]/g, '')).pipe(z.string().regex(/^[A-Za-z0-9_#.:-]{6,512}$/)) }).strict().parse(await c.req.json());
