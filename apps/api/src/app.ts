...
export function createApp(deps: Deps) {
 ...
 app.get('/connections/shopify/callback', async (c) => {
  ...
  return c.redirect(await deps.shopifyConnections.finish(new URL(c.req.url).searchParams, cookie, c.get('requestId')));
	...
	signedIn.get('/v1/organisations/:id/briefs/latest', async c => c.json(await new BriefService(deps.db).latest({ userId: c.get('session').userId, requestId: c.get('requestId') }, z.uuid().parse(c.req.param('id')), deps.workflows?.runnerProblem('morning-brief') ?? 'The workflow runner is stopped. Ask the operator to start it.')));
