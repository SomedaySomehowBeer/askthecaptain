...
it('a standalone task and its checklist stay out of any project, and follow the task into and out of one', async () => {
	...
	const step = await c.createTask(f.actor, f.org, { title: 'Order the filter', parentId: task.id });
	...
	assert.equal((await c.updateTask(f.actor, f.org, task.id, { projectId: plant.id })).projectId, plant.id);
	...
	assert.equal((await c.updateTask(f.actor, f.org, task.id, { title: 'Service the glycol chiller' })).projectId, plant.id, 'an absent projectId keeps the project');
	assert.equal((await c.updateTask(f.actor, f.org, task.id, { projectId: null })).projectId, null);
	...
	await assert.rejects(c.updateTask(f.actor, f.org, step.id, { projectId: plant.id }), { code: 'step_project' });
	await assert.rejects(c.createTask(f.actor, f.org, { title: 'Elsewhere', parentId: task.id, projectId: plant.id }), { code: 'step_project' });
	...
	await c.updateTask(f.actor, f.org, task.id, { status: 'done' });
...
it('a task can leave an archived project but cannot join one, and another tenant\'s project is invalid', async () => {
	...
	await c.updateProject(f.actor, f.org, old.id, { archived: true });
	await assert.rejects(c.createTask(f.actor, f.org, { title: 'Late', projectId: old.id }), { code: 'project_invalid' });
	assert.equal((await c.updateTask(f.actor, f.org, task.id, { projectId: null })).projectId, null);
	await assert.rejects(c.updateTask(f.actor, f.org, task.id, { projectId: old.id }), { code: 'project_invalid' });
...
it('series without a project materialise standalone occurrences; moving a series changes future occurrences only', async () => {
	...
	assert.equal((await c.updateSeries(f.actor, f.org, excise.id, { title: 'Excise duty return' })).projectId, compliance.id, 'an absent projectId keeps the project');
	assert.equal((await c.updateSeries(f.actor, f.org, excise.id, { projectId: null })).projectId, null);
...
it('the routine skips series in archived projects, never standalone ones, and discovers tenants with either', async () => {
	...
	await c.updateProject(archivedOnly.actor, archivedOnly.org, shelved.id, { archived: true });
...
it('Work and tags treat standalone tasks as eligible, and still exclude archived-project tasks and steps', async () => {
	...
	await c.createTask(f.actor, f.org, { title: 'A step', parentId: loose.id });
	...
	await c.updateProject(f.actor, f.org, archived.id, { archived: true });
