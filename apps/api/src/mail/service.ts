...
export class MailService {
	readonly #db: Sql; readonly #sync?: MailSync; readonly #automatic: boolean;
	constructor(db: Sql, sync?: MailSync, automatic = false) { this.#db = db; this.#sync = sync; this.#automatic = automatic; }
	async list(actor: Actor, organisationId: string, since?: string, limit = 50, before?: [string, string]) {
		return withTenant(this.#db, { organisationId, userId: actor.userId }, async (tx) => {
			...
			return { automaticSyncEnabled: this.#automatic, nextBefore: threads.length > limit ? `${edge!.sentAt.toISOString()}|${edge!.id}` : null, connection: conn, timezone: org!.timezone as string, lastSync: lastSync ?? null, threads: threads.slice(0, limit), hasMore: threads.length > limit };
		});
	}
