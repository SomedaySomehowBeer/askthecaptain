...
export async function saveThread(tx: TransactionSql, organisationId: string, conn: MailConnection, thread: MailThread) {
	...
	for (const m of thread.messages) {
		const [message] = await tx`insert into mail_messages (organisation_id, connection_id, thread_id, provider_id, from_header, to_header, cc_header, bcc_header, subject,
			date_header, sent_at, snippet, label_ids, in_reply_to, body, body_unavailable, rfc_message_id, list_unsubscribe, list_id, precedence, auto_submitted)
			values (${organisationId}, ${conn.id}, ${stored!.id}, ${m.providerId}, ${m.fromHeader}, ${m.toHeader}, ${m.ccHeader}, ${m.bccHeader}, ${m.subject},
				${m.dateHeader}, ${m.sentAt}, ${m.snippet}, ${tx.array(m.labelIds)}, ${m.inReplyTo}, ${m.body}, ${m.bodyUnavailable}, ${m.rfcMessageId ?? ''}, ${m.listUnsubscribe ?? false}, ${m.listId ?? ''}, ${m.precedence ?? ''}, ${m.autoSubmitted ?? ''})
			...
			body = excluded.body, body_unavailable = excluded.body_unavailable, rfc_message_id = excluded.rfc_message_id,
			list_unsubscribe = excluded.list_unsubscribe, list_id = excluded.list_id, precedence = excluded.precedence, auto_submitted = excluded.auto_submitted returning id`;
