...
async function setup() {
	...
	let mode = 'initial'; let failThread = ''; let historyExpired = false; let renamed = false; let held: Promise<void> | undefined;
	const calls: URL[] = [];
	...
	return { org: org.id, conn: conn!.id, sync, request, ownerToken, memberToken, strangerToken, calls,
		mode: (value: string) => { mode = value; }, fail: (id: string) => { failThread = id; }, expire: () => { historyExpired = true; }, rename: () => { renamed = true; }, hold: (promise: Promise<void>) => { held = promise; } };
}
...
it('partial failures roll back mail and cursor; history 404 performs a full recent resync', async () => {
