...
export class ChaseService {
 ...
 register(registry: Registry, push: PushService) {
 ...
 async notify(push: PushService, ctx: HandlerContext, args: Record<string, unknown>, phase: 'before' | 'after') {
  ...
  const recipient = await withTenant(this.db, ctx, async tx => {
   ...
   if (!member) throw new WorkflowPause('The task owner is no longer an active member. Reassign the task in Work, then Resume.');
   return { userId, task };
  });
  if ('skipped' in recipient) return recipient;
  const deliveries = await push.send(ctx.organisationId, recipient.userId, { title: `${phase === 'after' ? 'Overdue: ' : 'Due soon: '}${recipient.task.title}`, body: `Due ${recipient.task.due}. Open Work to review it.`, url: '/work?owner=all', tag: `chase:${ctx.organisationId}:task:${recipient.task.id}` }, { runId: ctx.runId, actor: { userId: ctx.userId, requestId: ctx.runId } });
