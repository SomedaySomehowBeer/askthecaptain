...
export class XeroService {
 ...
 async read(actor: Actor, org: string, overdueDays?: number, offset = 0) {
  return withTenant(this.db, { organisationId: org, userId: actor.userId }, async (tx) => {
   ...
   const [clock] = await tx`select (current_timestamp at time zone timezone)::date::text as today from organisations where id = ${org}`; const today = clock!.today as string;
   if (overdueDays !== undefined) {
    const rows = await tx`select i.provider_id, i.number, i.reference, i.due_date::text, i.currency, i.amount_due::text, i.total::text, i.amount_paid::text, c.name as contact_name, c.email as contact_email
