...
test.describe('stocktake', () => {
 ...
 test('an owner starts a location stocktake, a member counts, and Activity shows the completed run', async ({ page, context }) => {
  ...
  await context.addCookies([{ name: 'captain_session', value: member!, url: webUrl() }]); await page.goto(`${webUrl()}/resources/inventory`);
  ...
  await page.goto(`${webUrl()}/work?owner=all`); await expect(page.locator('.work-task').filter({ hasText: `Reorder ${itemName} (2 bags left, reorder at 5)` }).first()).toBeVisible();
 });
});
