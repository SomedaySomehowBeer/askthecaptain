...
(async () => {
 ...
 try {
  ...
  const initialReadCount=(await (await fetch('http://127.0.0.1:8084/__fixture/stats')).json()).reservationReadCount;
  ...
  assert.ok(new Set(stats.reservationReads.filter(r=>r.sequence>initialReadCount).map(r=>r.from)).size<=2,'initial read is lazy');
  ...
  const afterFuture=(await (await fetch('http://127.0.0.1:8084/__fixture/stats')).json()).reservationReads.filter(r=>r.sequence>initialReadCount);
  ...
  await expect(page.getByRole('region',{name:'A Fermenter reservations'})).toContainText('Availability is unknown');
  const failedCount=(await (await fetch('http://127.0.0.1:8084/__fixture/stats')).json()).reservationReadCount;
  await page.waitForTimeout(1500);
  assert.equal((await (await fetch('http://127.0.0.1:8084/__fixture/stats')).json()).reservationReadCount,failedCount,'no automatic failure loop');
