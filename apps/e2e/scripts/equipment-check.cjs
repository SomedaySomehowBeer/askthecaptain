...
(async () => {
 ...
 try {
  ...
  await page.getByLabel('Project (optional)',{exact:true}).selectOption(fixture.projectId);await page.getByLabel('Task (optional)',{exact:true}).selectOption(fixture.tasks['Confirm packaging slot']);
  ...
  assert.equal(stored.setupMinutes,15);assert.equal(stored.taskId,fixture.tasks['Confirm packaging slot']);
