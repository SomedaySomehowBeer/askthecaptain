...
(async () => {
 ...
 try {
  ...
  await standalone.getByText('Add recurring work', { exact: true }).click();
  const form = standalone.locator('form').filter({ has: page.getByLabel('First period starts') });
  await form.getByLabel('Title', { exact: true }).fill('Standalone recurring inspection');
  ...
  await expect(standalone.locator('.line').filter({ hasText: 'Standalone recurring inspection' })).toBeVisible();
  const overview = await api('/commitments');
  const series = overview.series.find(s => s.title === 'Standalone recurring inspection'); assert.equal(series.projectId, null);
