...
}, (t) => [unique().on(t.organisationId, t.connectionId, t.providerId), unique().on(t.organisationId, t.connectionId, t.id),
...
export const mailMessages = pgTable('mail_messages', {
	...
	fromHeader: text('from_header').notNull(), toHeader: text('to_header').notNull(), ccHeader: text('cc_header').notNull(), subject: text('subject').notNull(),
