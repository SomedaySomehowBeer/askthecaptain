...
-- Like every policy and grant since 0041, these name both runtime roles (D6).
...
create policy tenant on engine_spike.outbox for all to app, captain_runtime using (organisation_id = current_organisation_id()) with check (organisation_id = current_organisation_id());
alter table engine_spike.labels enable row level security;
alter table engine_spike.labels force row level security;
create policy tenant on engine_spike.labels for all to app, captain_runtime using (organisation_id = current_organisation_id()) with check (organisation_id = current_organisation_id());
alter table engine_spike.faults enable row level security;
alter table engine_spike.faults force row level security;
create policy tenant on engine_spike.faults for all to app, captain_runtime using (organisation_id = current_organisation_id()) with check (organisation_id = current_organisation_id());
grant usage on schema engine_spike to app, captain_runtime;
grant select, insert, update on all tables in schema engine_spike to app, captain_runtime;
