...
- The history baseline is captured before listing recent mail, so arrivals during the initial pass
  ...
  A Gmail history 404 starts a full recovery. Other failures roll back all mail/cursor writes and
  record a content-free `mail.sync_failed` system audit entry. Retry with **Sync now**.
- `mail.synced` records system counts and whether the pass was full or capped. Each organisation has an
  in-process overlap guard and a Postgres advisory lock. Token refresh uses the connection row lock.
