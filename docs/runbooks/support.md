...
| "Sync says it did not finish" | The `Reference:` at the end of the Inbox or Calendar card: stage, kind, status, Google's reason or a database code | Read it with [mail-sync.md](mail-sync.md#diagnosing-a-failed-sync): `accessNotConfigured` is an API the owner has not enabled in Google Cloud, a bare 403 is a missing scope (reconnect), rate limits wait, `database` codes that repeat on the same stage become an issue with the reference, never the mail. |
