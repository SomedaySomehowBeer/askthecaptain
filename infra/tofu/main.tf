terraform {
  ...
  required_providers {
    ...
    # Kept for one apply so tofu can destroy the KMS key and IAM user it still holds in state (D16);
    # remove this line, the aws provider block and the AWS workflow secrets after that apply.
